...
async function agregarAdministrador(celdas){
    ...
    if(inputsFiltrado.length !== 0){
        ...
        for(let i = 0; i<inputsFiltrado.length; i++){
            ...
            const urlAniadir = `http://localhost/Sitio_Liceo_Nº1/PHP/Opciones/Administrador.php?consulta=aniadirAdministrador`;
            let respuesta = await fetch(urlAniadir, config);
            datos = await respuesta.text();
        }

        if(datos === "Duplicado"){
            alert("Administrador ya existente en la base de datos, use otra cédula o elimine el existente");
        }else if(datos === "Agregado"){
            alert("Administrador creado correctamente");
        }else{
            alert("Error al crear al administrador");
        }
    }else{
        alert("Ingrese datos para agregar un administrador");
    }
}
...
async function eliminar() {
    ...
    if(seleccionados.length === 0){
        alert("Seleccione un checkbox para eliminar un administrador");
    } else{
        for (let i = 0; i < seleccionados.length; i++) {
            Tr[i] = seleccionados[i].closest('tr');
        }

        let confirmar = confirm('¿Estas seguro que quieres eliminar los administrador seleccionados?');
        if(confirmar){
            //Manda las consultas de las filas del checked
            Tr.forEach(async (tr) => {

                const url = `http://localhost/Sitio_Liceo_Nº1/PHP/Opciones/Administrador.php?consulta=eliminarAdministrador&cedula=${tr.cells[0].textContent}`;
                let respuesta = await fetch(url);

                tr.remove();
            });
            alert("Se elimino correctamente al administrador");
...
async function modificarAdministrador(filas){
    ...
    if(filasInput.every(fila => fila.children[0].value.trim() !== '')){
        ...
        const urlModificar = `http://localhost/Sitio_Liceo_Nº1/PHP/Opciones/Administrador.php?consulta=modificarAdministrador`;
        let respuesta = await fetch(urlModificar, config);
    }else{
        alert("Ingrese los datos para modificar un administrador");
    }
}
...
function modificar(event) {
    ...
    if(clickModificar){
        ...
        modificarAdministrador(tdInputs);
