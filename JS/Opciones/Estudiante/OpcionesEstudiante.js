...
async function agregarProfesor(celdas){
    ...
    if(inputsFiltrado.length !== 0){
        ...
        for(let i = 0; i<inputsFiltrado.length; i++){
            ...
            const urlAniadir = `http://localhost/Sitio_Liceo_Nº1/PHP/Opciones/Estudiante.php?consulta=aniadirEstudiante`;
            let respuesta = await fetch(urlAniadir, config);
            datos = await respuesta.text();
        }

        if(datos === "Duplicado"){
            alert("Estudiante ya existente en la base de datos, use otra cédula o elimine el existente");
        ...
        alert("Ingrese datos para agregar un estudiante");
    }
}
...
async function eliminar() {
    ...
    if(seleccionados.length === 0){
        alert("Seleccione un checkbox para eliminar un estudiante");
    } else{
        for (let i = 0; i < seleccionados.length; i++) {
            Tr[i] = seleccionados[i].closest('tr');
        }

        let confirmar = confirm('¿Estas seguro que quieres eliminar los estudiante seleccionados?');
        if(confirmar){
            //Manda las consultas de las filas del checked
            Tr.forEach(async (tr) => {

                const url = `http://localhost/Sitio_Liceo_Nº1/PHP/Opciones/Estudiante.php?consulta=eliminarEstudiante&cedula=${tr.cells[0].textContent}`;
                let respuesta = await fetch(url);

                tr.remove();
            });
            alert("Se elimino correctamente al estudiante");
...
async function modificarProfesor(filas){
    ...
    if(filasInput.every(fila => fila.children[0].value.trim() !== '')){
        ...
        const urlModificar = `http://localhost/Sitio_Liceo_Nº1/PHP/Opciones/Estudiante.php?consulta=modificarEstudiante`;
        let respuesta = await fetch(urlModificar, config);
    }else{
        alert("Ingrese los datos para modificar un estudiante");
    }
}
